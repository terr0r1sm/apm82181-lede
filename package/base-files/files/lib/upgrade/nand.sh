...
nand_upgrade_tar() {
	...
	local kernel_length=`tar Oxf $tar_file $kernelfile | wc -c) 2> /dev/null`
	local rootfs_length=`tar Oxf $tar_file $rootfsfile | wc -c) 2> /dev/null`

	local rootfs_type="$(identify_tar "$tar_file" $rootfsfile)"

	local has_kernel=1
	local has_env=0

	[ "$kernel_length" != 0 -a -n "$kernel_mtd" ] && {
		tar Oxf $tar_file $kernelfile | mtd write - $CI_KERNPART
	}
	...
	[ "$has_kernel" = "1" ] && {
		local kern_ubivol="$(nand_find_volume $ubidev $CI_KERNPART)"
		tar Oxf $tar_file $kernelfile | \
			ubiupdatevol /dev/$kern_ubivol -s $kernel_length -
	}

	local root_ubivol="$(nand_find_volume $ubidev rootfs)"
	tar Oxf $tar_file $rootfsfile | \
